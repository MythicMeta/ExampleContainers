...
var payloadDefinition = agentstructs.PayloadType{
	...
	SupportedOS:                            []string{agentstructs.SUPPORTED_OS_LINUX, "customOS"},
	Wrapper:                                false,
...
func build(payloadBuildMsg agentstructs.PayloadBuildMessage) agentstructs.PayloadBuildResponse {
	...
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	// some extra fake stuff you can do
	if callback, err := mythicrpc.SendMythicRPCCallbackCreate(mythicrpc.MythicRPCCallbackCreateMessage{
		PayloadUUID:   payloadBuildMsg.PayloadUUID,
	...
	// run is going to fail because it's not poseidon
